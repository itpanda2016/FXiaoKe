            ...
            if (retToken.errorCode == 0) {
                ...
                if (retDpts.errorCode == 0) {
                    foreach (var item in retDpts.departments) {
                        //todo 学习递归
                        //按ID排序，取上级为0或是没有上级的，然后此级为公司，以此ID，取出所有上级ID为该ID的（如果无，说明没有下级了）；再继续循环取出以这些ID为父级的下级ID
                        //其实就是递归：http://www.cnblogs.com/Ferry/archive/2010/12/14/1905283.html
                        //更简洁的递归：http://blog.csdn.net/kone0611/article/details/43053455
